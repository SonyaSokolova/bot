...
bot = TeleBot(token='6820008992:AAFEtbdKKHctC5ckJfOKsnIlfe6zJUUaiuI', parse_mode='html') # создание бота

# словарь с определениями и аббревиатурами, которые знает бот
# в формате:
# 'ключевая фраза': 'соответствующее ей определение'
DEFINITOINS = {
    'регресс': 'это проверка что новый функционал не сломал существующий',
    ...
    'cookie': ' это данные, которые браузер хранит на своей стороне. Информация в cookie позволяет сайтам «запоминать» своих посетителей, например, чтобы каждый раз не переспрашивать их логин и пароль.',
    'кса': 'это один из вариантов устройства современных цифровых продуктов, в котором мы делим продукт на три части: клиент, сервер и база данных',
    'rest': 'это архитектурный стиль, набор рекомендаций',
...
def start_command_handler(message: types.Message):
    ...
    bot.send_message(
        ...
        text='Привет!Я помогу тебе расшифровать новые термины. Введи термин который интересен.'
